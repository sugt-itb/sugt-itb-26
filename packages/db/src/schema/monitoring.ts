 ...
 * tab (ADR-0028's Editor Grant gates writing them). A Card is a title, a date or date-range, and a
 ...
 *
 * A Card carries **no Jenis** — the category is per checklist item (#292). `title` carries a non-empty
 * CHECK as a backstop; the write validates it first and returns a value outcome, so the CHECK only
 * ever refuses a hand-written row.
 */
export const preparationCard = pgTable(
  ...
  (t) => [check("preparation_card_title_not_empty", sql`length(trim(${t.title})) > 0`)],
);
 ...
 *
 * `jenis` is the item's category, moved down from the Card (#292): it CHECKs the four
 * `PREPARATION_JENIS` values character for character. **Its `Pimpinan` value is a category label, not
 * the Person Role** — an item's Jenis never gates access; only the Monitoring Editor Grant does.
 *
...
export const preparationChecklistItem = pgTable(
  ...
  (t) => [
    ...
    check(
      "preparation_checklist_item_jenis_check",
